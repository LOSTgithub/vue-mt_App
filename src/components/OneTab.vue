<template>
  <div class="one-tab-wrapper" ref="oneTab">
    <!-- 一级导航 -->
    <div class="tab-item"  v-for="(img, ind) in menuList"
     :key="ind"
     ...
     @touchmove="move = true">
      ...
      <!-- 文字 -->
      <div class="tab-title">{{img.title}}</div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    scrollTo (i, e) {
      ...
      // 获取侧边栏数据
      // 这个地方传了一个参数
      this.getSideListt(this.menuList[i].title)
    }
  },
  mounted () {
    this.getSideListt(this.menuList[0].title)
  },
}
</script>
